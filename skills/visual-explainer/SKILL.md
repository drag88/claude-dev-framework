...
description: Generates beautiful, self-contained HTML pages that visually explain systems, code changes, plans, and data. Activates when the user asks for a diagram, architecture overview, diff review, plan review, project recap, comparison table, or any visual explanation of technical concepts. Also activates proactively when about to render a complex ASCII table (4+ rows or 3+ columns) — presents it as a styled HTML page instead.
license: MIT
...
**What type of content?** Architecture, flowchart, sequence, data flow, schema/ER, state machine, mind map, class diagram, C4 architecture, data table, timeline, dashboard, or prose-first page. Each has distinct layout needs and rendering approaches.

> See `references/diagram-types.md` for detailed instructions per diagram type (architecture, flowcharts, sequence, ER, state machines, mind maps, class diagrams, C4, data tables, timelines, dashboards, implementation plans, documentation, prose accents).
...
Share pages via Vercel: `bash {{skill_dir}}/scripts/share.sh <html-file>`. No account required — deployments are public, live immediately, with 30-day retention. See `./commands/share.md` for the `/share` command template.
...
- **No overflow**: Every grid/flex child needs `min-width: 0`. Side-by-side panels need `overflow-wrap: break-word`. See Overflow Protection in `./references/css-patterns.md`.
- **Mermaid zoom controls**: Every `.mermaid-wrap` needs zoom controls (+/-/reset/expand), Ctrl/Cmd+scroll zoom, click-and-drag panning, click-to-expand. See `./references/css-patterns.md`.
...
**Forbidden patterns:** See detailed lists under Typography, Color Palette, Section Headers, Layout & Hierarchy, and Template Patterns in the anti-slop rules above.

## Reference Files

| File | Contents |
|------|----------|
| `references/diagram-types.md` | Per-type instructions: architecture, flowcharts, sequence, ER, state machines, mind maps, class diagrams, C4, data tables, timelines, dashboards, implementation plans, documentation, prose accents |
| `references/css-patterns.md` | CSS/layout patterns, SVG connectors, overflow protection, depth tiers, code blocks, prose elements, image containers |
| `references/libraries.md` | Font pairings, Mermaid theming guide, Chart.js setup, anime.js, CDN imports |
| `references/responsive-nav.md` | Sticky sidebar TOC (desktop) + horizontal scrollable bar (mobile) for multi-section pages |
| `references/slide-patterns.md` | Slide engine CSS, 10 slide types, transitions, nav chrome, curated presets |
