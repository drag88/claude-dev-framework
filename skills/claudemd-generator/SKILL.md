...
## Design Principles (current-Claude-aligned, from the vendored rulebook + Anthropic docs)
...
- **No duplication** — if content lives in `.claude/rules/` (auto-loaded), do not repeat it. Conflicts between files cause the model to pick one arbitrarily.
- **Concrete and verifiable** — "Run `npm test` and paste the output" beats "test your changes." Vague checks get silently skipped.
- **No code style** — let linters handle formatting.
- **Open with a Role sentence** — One sentence anchoring tone and scope ("You are a senior X engineer working on Y...").
- **No tool/subagent authorization boilerplate** — current models parallelize and dispatch subagents natively. Do not emit `<use_parallel_tool_calls>` blocks or a "Tool and subagent policy" section; deleting such blocks is the upgrade.
- **Spend the tokens on gotchas** — invariants, traps, incident-derived knowledge, each with the why. Never restate what `ls` or package.json shows.
- **Frame rules as positive imperatives** — "Use X" beats "Never Y." Prohibitions waste tokens and invite pattern-matching.
...
- **Tone down forceful language** — Avoid CAPS, MUST, CRITICAL, "ANY", "ALWAYS". Forceful negatives cause overcompliance and hostile reading. Reserve absolutes for security/secrets/irreversible operations.
- **Provide the why** behind non-obvious constraints — models generalize correctly from explained constraints.
...
| Gotchas (the non-obvious only, each with the why) | `CLAUDE.md` | The section that earns the file its tokens |
| Model Routing (Fable orchestrates; Opus reasoning, Sonnet mechanical, Codex adversarial review) | `CLAUDE.md` | Model choice is a per-task decision; needs guaranteed load |
...
| CDF tools available (routing table) | `CLAUDE.md` | Without explicit routing, Claude falls back to generic approaches instead of `/cdf:troubleshoot`, `/cdf:verify`, codebase-navigator, etc. — defeats the point of having CDF installed |
...
**Step 0 of every generation**: Read `rules-templates/claudemd-rulebook.md` (vendored into CDF). Treat it as the single source of truth for current-model rules, anti-patterns, the audit checklist, and the recommended skeleton. The Design Principles above become a fallback baseline only if the rulebook file is missing.
...
**Updating the rulebook**: Edit `rules-templates/claudemd-rulebook.md` directly against Anthropic's current prompting and context-engineering docs, and commit. The CDF version bump tracks the rulebook update.

**Generation flow:**
1. Read `rules-templates/claudemd-rulebook.md` as the rulebook. If missing, fall back to the embedded principles above.
