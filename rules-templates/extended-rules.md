...
> **Status:** opt-in reference. CDF's default generator (`/cdf:rules claudemd`) ships a 6-rule Critical Rules section, not this 12-rule set. Import individual rules from here only when you have a project-specific reason. Three rules in the source set are anti-patterns on current Claude models — those are flagged below.

This file is a community 12-rule CLAUDE.md template that has circulated as a starting point. CDF retains it for reference and selective adoption. Each rule below carries a verdict against the [vendored rulebook](claudemd-rulebook.md) and the gist's coding-agent best practices ([skills/tuning-coding-agent-codebases/SKILL.md](../skills/tuning-coding-agent-codebases/SKILL.md)).
...
- **Reject** — actively harmful on current Claude models (forced summaries, unenforceable budgets, wrong layer).
...
**Why skip:** Duplicates CDF's Critical Rule #1 (Read before edit) and the plan-mode default in `workflow.md`. Current Claude models handle ambiguity surfacing natively.
...
**Why skip:** The scope-discipline intent is fine, but the phrasing ("don't refactor what isn't broken") overlaps with the gist's old-model-compensating grep (phrase #7: "Do not refactor existing code"). Current models don't drift into adjacent code the way older ones did. Conformance is already implicit in CDF Critical Rule #1.

### Rule 4 — Goal-Driven Execution *(Skip)*
Define success criteria. Loop until verified.

**Why skip:** Vague. Current Claude models loop to verification by default. Adds noise without changing behavior.
...
**Why reject:** The model cannot observe token counts mid-turn. The context window is large; a 30K session cap is below normal working size. This rule hallucinates a control surface that does not exist. Token budgets, if enforced, belong in client-side telemetry, not in CLAUDE.md.
...
**Why keep:** Genuinely additive. Aligns with rulebook rule 20 (audit for conflicts across files). Current Claude models will silently average plausible contradictions if not told otherwise.
...
**Why reject:** This is the **forced interim summary** pattern. The rulebook (rule 18, "Delete forced interim-progress scaffolding") and the gist both flag this as a delete-on-sight pattern. Current models' native progress updates already cover this. Forced summaries burn tokens and don't improve outcomes.
...
**Why keep:** Genuinely additive. Counters current models' mild tendency to declare success when one step was silently bypassed. Pairs with the failure-recovery skill.
...
| Reject (anti-pattern on current Claude models) | 5, 6, 10 |
...
- The vendored `claudemd-rulebook.md` in this directory
